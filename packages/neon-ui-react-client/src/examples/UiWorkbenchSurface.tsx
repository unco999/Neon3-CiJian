...
export function UiWorkbenchSurface({ snapshot }: { snapshot: UiWorkbenchSnapshot }) {
  ...
  return <Surface surfaceId="surface.ui-workbench" revision={snapshot.revision} bounds={{ x: 0, y: 0, width: 1440, height: 900 }} style={shell}>
    <Panel nodeKey="workspace" bounds={{ x: 12, y: 12, width: 1416, height: diagnosticsY - 24 }} style={panel}>
      ...
      <Button nodeKey="toggle-diagnostics" bounds={{ x: 1210, y: 12, width: 180, height: 32 }} style={snapshot.diagnosticsExpanded ? active : panel} event={{ type: "DIAGNOSTICS_TOGGLE" }}>{snapshot.diagnosticsExpanded ? "Collapse diagnostics" : "Expand diagnostics"}</Button>
      {(["overview", "materials", "history"] as const).map((tab, index) => <Button key={tab} nodeKey={`tab-${tab}`} bounds={{ x: 16 + index * 116, y: 60, width: 108, height: 30 }} style={snapshot.inspectorTab === tab ? active : panel} event={{ type: "INSPECTOR_TAB_SELECT", tab }}>{tab}</Button>)}
