...
fn emit(sequence: u64, method: &str, producer: serde_json::Value, consumer: serde_json::Value, pass: bool, error: Option<String>) {
    println!("{}", json!({
...
fn main() {
    let document = match parse_nui_flow(FLOW) {
        ...
        Err(error) => {
            emit(1, "parse", json!({"source_bytes": FLOW.len()}), json!({}), false, Some(format!("{error:?}")));
            std::process::exit(1);
        }
    };
    let ir = lower_nui_flow(&document);
    emit(1, "parse", json!({"surface_id": "surface.agents", "revision": ir.revision}), json!({"node_count": count_nodes(&ir.root)}), true, None);
        ...
        operations: vec![
            ...
            UiPatchOp::InsertNode {
                ...
                node: neon_ui_schema::UiNode {
                    ...
                    bounds: neon_ui_schema::UiBounds { x: 0.0, y: 0.0, width: 640.0, height: 48.0 },
    ...
    let pass = patched.as_ref().is_ok_and(|value| value.revision.0 == 8 && count_nodes(&value.root) == 4);
    emit(2, "ui.patch.apply", json!({"base_revision": 7, "operation_count": 2}), patched.as_ref().map(|value| json!({"revision": value.revision, "node_count": count_nodes(&value.root)})).unwrap_or_default(), pass, patched.as_ref().err().map(|error| format!("{error:?}")));

    let stale = apply_ui_patch(&ir, &UiPatch { base_revision: 6, ..patch });
    let stale_pass = stale.is_err();
    emit(3, "ui.patch.stale", json!({"base_revision": 6, "current_revision": 7}), json!({"rejected": stale_pass}), stale_pass, stale.err().map(|error| format!("{error:?}")));

    let final_pass = pass && stale_pass;
    println!("{}", json!({"probe": "ui-patch-contract.v1", "final": true, "pass_result": final_pass}));
    if !final_pass {
        std::process::exit(1);
    }
}
