...
use neon_ui_schema::{
    ...
    UI_PROGRAM_TEXT_REGISTRY_CAPABILITY_NAME, UiCpuViewport, UiInputChange, UiInputFrame,
    UiInputValue, UiProgramCapability, UiProgramCapabilityOwner, UiProgramCapabilityStatus,
    UiProgramResource, UiProgramResourceKind, UiProgramRevision, UiRepeatFrame, UiRepeatRow,
};
...
pub fn terrain_workbench_program() -> neon_ui_schema::UiProgram {
    compile_nui_flow_program(
        &terrain_workbench_document(),
        terrain_workbench_program_revision(),
    )
...
pub fn terrain_workbench_input_frame(
    expected_input_revision: Revision,
    request_id: &str,
    changes: Vec<UiInputChange>,
) -> UiInputFrame {
    UiInputFrame {
        program_revision: terrain_workbench_program_revision(),
        expected_input_revision,
        request_id: request_id.into(),
        idempotency_key: format!("workbench:{}", request_id),
        changes,
    }
}
...
mod tests {
    use super::*;
    use crate::{
        UiInputStore, UiInputWriter, UiLocalPresentationState, UiRepeatStore, UiTextRegistry,
        apply_nui_ir_patch, evaluate_ui_program, parse_nui_flow_patch,
    };
    use neon_ui_schema::{
        UiProgramSemanticEvent, UiProgramSemanticEventKind, UiProgramSemanticEventStatus,
        UiSemanticInteractionMetadata, UiTextSourceCategory,
    };
    ...
    fn terrain_workbench_default_update_branch_repeat_and_patch_scenarios_are_headless() {
        ...
        .unwrap();
        let defaults = evaluate_ui_program(
            &program,
            &store.snapshot(),
            terrain_workbench_viewport(1440.0, 900.0, 1),
        ...
        assert!(
            defaults
                .nodes
                .iter()
                .any(|node| node.node_key == "terrain-render-surface")
        );
        assert!(
            defaults
                .nodes
                .iter()
                .any(|node| node.node_key == "loading-state" && node.visible)
        );

        let update = terrain_workbench_input_frame(
            Revision(0),
            "domain-ready",
            vec![
                UiInputChange {
                    key: "workbench_state".into(),
        ...
        let ready = evaluate_ui_program(
            &program,
            &store.snapshot(),
            terrain_workbench_viewport(720.0, 900.0, 2),
            &UiLocalPresentationState::default(),
        );
        assert!(
            ready
                .nodes
                .iter()
                .any(|node| node.node_key == "ready-state" && node.visible)
        );
        assert!(
            ready
                .nodes
        ...
        let mut events =
            crate::UiProgramSemanticEventRouter::new(program.clone(), store.snapshot(), 7);
        let activate = UiProgramSemanticEvent {
            event_id: "tool-select".into(),
            kind: UiProgramSemanticEventKind::Activate,
            intent: "terrain.tool.select".into(),
            source_node_key: "tool-select".into(),
        ...
        assert_eq!(
            events.validate(&activate).status,
        ...
        assert_eq!(
            events.validate(&stale_event).status,
            UiProgramSemanticEventStatus::Rejected
        );

        let mut repeats = UiRepeatStore::default();
        let rows = repeats
            .apply(&program, terrain_workbench_material_rows(1, 9))
            .unwrap();
        ...
        assert_eq!(
            apply_nui_ir_patch(&document.ir, &patch).unwrap().revision,
            Revision(2)
        );
    }
    ...
    fn terrain_workbench_dynamic_text_uses_replacement_handles_not_frame_strings() {
        ...
        let title = registry
            .insert_dynamic(Revision(0), "Terrain Alpha".into())
            .unwrap();
        let mut store =
            UiInputStore::activate(terrain_workbench_program_revision(), document.input_schema)
                .unwrap();
        let update = terrain_workbench_input_frame(
            Revision(0),
            "title",
            vec![UiInputChange {
                key: "project_title".into(),
                value: UiInputValue::TextHandle { value: title },
            }],
        );
        store
            .apply_with_text_registry(UiInputWriter::External, update, &registry)
            .unwrap();
        registry
            .replace_dynamic(Revision(1), title, "Terrain Beta".into())
            .unwrap();
        assert_eq!(
            registry.handle_diagnostic(title).status,
            neon_ui_schema::UiTextHandleStatus::Ready
        );
        assert_eq!(
            registry.snapshot(true).records[0].category,
            UiTextSourceCategory::Dynamic
        );
    }
}
