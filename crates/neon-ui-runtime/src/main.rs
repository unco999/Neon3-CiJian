...
fn main() {
    ...
    if args
        .get(1)
        .is_some_and(|argument| argument == "--forward-server")
    {
        let endpoint = args
            .get(2)
            .expect("UI runtime endpoint is required")
            .parse()
            .expect("UI runtime endpoint must be a socket address");
        let wgpu_endpoint = args
            .get(3)
            .expect("WGPU endpoint is required")
            .parse()
            .expect("WGPU endpoint must be a socket address");
        let domain_endpoint = args
            .get(4)
