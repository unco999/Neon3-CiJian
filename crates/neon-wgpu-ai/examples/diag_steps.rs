...
fn stats(name: &str, data: &[f32]) {
    ...
    println!(
        "{name}: n={} nan={} inf={} min={lo} max={hi} mean={mean:.4}",
        data.len(),
        nan,
        inf
    );
...
fn main() {
    ...
    for (k, n) in [(8usize, 4usize), (8usize, 32usize)] {
        ...
        let weight_kn: Vec<f32> = (0..k * n).map(|i| weight_nk[(i % n) * k + i / n]).collect();
        ...
        for (tb, v) in [
            (
                true,
                ops::matmul_t(ctx, &mbuf, &wbuf_nk, 1, n as u32, k as u32, false, true),
            ),
            (
                false,
                ops::matmul_t(ctx, &mbuf, &wbuf_kn, 1, n as u32, k as u32, false, false),
            ),
        ] {
            let v = ctx.readback_f32(&v.buffer, n).expect("mat out");
            let ok = v.iter().zip(&expect).all(|(a, b)| (a - b).abs() < 1e-2);
            println!(
                "matmul m=1 k={k} n={n} trans_b={tb}: ok={ok} got_first4={:?} expect_first4={:?}",
                &v[..n.min(4)],
                &expect[..n.min(4)]
            );
    ...
    for (got, input) in scores_gpu.chunks_exact(4).zip(scores_cpu.chunks_exact(4)) {
        ...
        for (actual, x) in got.iter().zip(input) {
            ...
            assert!(
                ...
                "softmax regression: {actual} != {expected}"
            );
        }
    }
    ...
    .expect("group norm");
    let gn_gpu = ctx
        .readback_f32(&gn.buffer, gn_input.len())
        .expect("group norm out");
    for (i, actual) in gn_gpu.iter().enumerate() {
        let expected = if i % 2 == 0 { -0.999995 } else { 0.999995 };
        assert!(
            (actual - expected).abs() < 1e-4,
            "group norm regression at {i}: {actual} != {expected}"
        );
    }
    ...
    let film = ops::film(
        ...
        &film_params_buf,
        2,
        film_input.len() as u64,
    );
    let film_gpu = ctx
        .readback_f32(&film.buffer, film_input.len())
        .expect("film regression out");
    let film_expected = [2.5f32, 4.0, 5.5, 5.5, 13.0, 20.5];
    for (index, (actual, expected)) in film_gpu.iter().zip(film_expected).enumerate() {
        assert!(
            (actual - expected).abs() < 1e-6,
            "FiLM NCHW regression at {index}: {actual} != {expected}"
        );
    }
    ...
    let ddim_gpu = ctx
        .readback_f32(&ddim.buffer, 4)
        .expect("ddim regression out");
    for (index, actual) in ddim_gpu.iter().enumerate() {
        ...
        assert!(
            (actual - expected).abs() < 1e-6,
            "DDIM regression at {index}: {actual} != {expected}"
        );
    }
    ...
    let noise_a_values = ctx
        .readback_f32(&noise_a.buffer, 64)
        .expect("noise seed 42");
    let noise_b_values = ctx
        .readback_f32(&noise_b.buffer, 64)
        .expect("noise seed 43");
    let noise_a_repeat_values = ctx
        .readback_f32(&noise_a_repeat.buffer, 64)
        .expect("noise seed 42 repeat");
    assert_ne!(
        noise_a_values, noise_b_values,
        "different seeds must produce different latent noise"
    );
    assert_eq!(
        noise_a_values, noise_a_repeat_values,
        "the same seed must be deterministic"
    );
    ...
    stats(
        "randn",
        &ctx.readback_f32(&noise.buffer, len as usize)
            .expect("readback randn"),
    );
    ...
    stats(
        "forward(cond)",
        &ctx.readback_f32(&ec.buffer, len as usize)
            .expect("readback ec"),
    );
    ...
    stats(
        "forward(null)",
        &ctx.readback_f32(&eu.buffer, len as usize)
            .expect("readback eu"),
    );
    ...
    stats(
        "cfg_combine",
        &ctx.readback_f32(&e.buffer, len as usize)
            .expect("readback e"),
    );

    let (sab_t, s1ab_t) = (
        model.schedule.sab[t as usize],
        model.schedule.s1ab[t as usize],
    );
    let (sab_t0, s1ab_t0) = (
        model.schedule.sab[t0 as usize],
        model.schedule.s1ab[t0 as usize],
    );
    ...
    stats(
        "ddim_step",
        &ctx.readback_f32(&next.buffer, len as usize)
            .expect("readback next"),
    );
