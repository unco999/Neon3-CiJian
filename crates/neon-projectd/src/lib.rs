...
use neon_protocol::{
    AssetBytes, AssetRef, HealthStatus, PROTOCOL_VERSION, ProjectSnapshot, ProjectSummary,
    RequestId, Revision, RpcError, RpcRequest, RpcResponse, RpcStatus, ServiceDescription,
    ServiceEvent, ServiceHealth, ServiceName, SubscriptionRequest, SubscriptionResponse,
};
...
impl Projectd {
    pub fn fixture(epoch: u64) -> Self {
        ...
        let font = AssetBytes {
            asset: font_asset.clone(),
            media_type: "font/ttf".into(),
            width: None,
            height: None,
            bytes: FIXTURE_SARASA_UI_SC_LIGHT_TTF.to_vec(),
        };
    ...
    pub fn service_description(&self) -> ServiceDescription {
        ServiceDescription {
            ...
            epoch: self.epoch,
            capabilities: vec![
                "project.summary.v1".into(),
                "asset.list.v1".into(),
                CAPABILITY_ASSET_BYTES.into(),
                CAPABILITY_PROJECT_SUBSCRIBE.into(),
                "debug.trace.query.v1".into(),
            ],
        }
    }
    ...
    pub fn handle(&mut self, request: RpcRequest) -> RpcResponse {
        ...
        let result = match request.method.as_str() {
            ...
            "debug.diagnostics.get" => Ok(
                json!({"project_id": self.project_id, "revision": self.revision, "asset_count": self.assets.len(), "event_sequence": self.current_sequence(), "journal_records": self.journal.records().len()}),
            ),
            "debug.command.get" => self.command_receipt(request.params),
            "debug.trace.query" | "debug.journal.query" => {
                Ok(json!(self.trace_query(request.params)))
            }
        ...
        match result {
            Ok(result) => {
                ...
                RpcResponse {
                    ...
                    result: Some(result),
                    snapshot: Some(json!(self.project_snapshot())),
                    error: None,
                }
            }
            Err((code, message)) => {
    ...
    fn publish_snapshot(&mut self) {
        ...
        self.events.push_back(ServiceEvent {
            epoch: self.epoch,
            sequence,
            payload: json!({"event": "project.snapshot.published", "snapshot": snapshot}),
        });
        self.next_event_sequence += 1;
    }

    fn subscribe(&self, params: Value) -> Result<Value, (&'static str, &'static str)> {
        let cursor: SubscriptionRequest = serde_json::from_value(params).map_err(|_| {
            (
                "invalid_request",
                "a subscription epoch and cursor are required",
            )
        })?;
        if cursor.epoch != self.epoch {
            return Err((
                "epoch_mismatch",
                "subscription cursor belongs to a previous service epoch",
            ));
        }
        if let (Some(from_sequence), Some(first)) = (cursor.from_sequence, self.events.front()) {
            if from_sequence.saturating_add(1) < first.sequence {
                return Err((
                    "subscription_cursor_expired",
                    "subscription cursor is older than retained events",
                ));
            }
        }
    ...
    fn command_receipt(&self, params: Value) -> Result<Value, (&'static str, &'static str)> {
        let request_id = params
            ...
            .ok_or(("invalid_request", "request_id is required"))?;
        self.receipts
            .get(&RequestId(request_id.into()))
            .map(|receipt| json!(receipt))
            .ok_or(("command_not_found", "command receipt is not retained"))
    }
    ...
    fn asset_bytes(&self, params: Value) -> Result<Value, (&'static str, &'static str)> {
        let asset: AssetRef = serde_json::from_value(params)
            .map_err(|_| ("invalid_request", "a stable AssetRef is required"))?;
        self.assets
            .get(&key(&asset))
            .map(|content| json!(content))
            .ok_or((
                "asset_revision_not_found",
                "asset bytes are not available at this revision",
            ))
    }
}
...
mod tests {
    ...
    fn request(id: &str, method: &str, params: Value) -> RpcRequest {
        RpcRequest {
            ...
            method: method.into(),
            params,
            expected_revision: Some(Revision(5)),
    ...
    fn snapshot_assets_subscription_and_receipts_are_revisioned() {
        ...
        let subscription = service.handle(request(
            "subscribe",
            "project.subscribe",
        ...
        let subscription: SubscriptionResponse =
            serde_json::from_value(subscription.result.unwrap()).unwrap();
        assert_eq!(subscription.next_sequence, 2);
        assert_eq!(subscription.events.len(), 1);
        assert_eq!(
            subscription.events[0].payload["event"],
            "project.snapshot.published"
        );

        let receipt = service.handle(request(
            "receipt",
            "debug.command.get",
            json!({"request_id": "summary"}),
        ));
    ...
    fn loopback_server_exposes_the_project_control_plane() {
        ...
        let description: ServiceDescription =
            serde_json::from_value(description.result.unwrap()).unwrap();
        ...
        assert_eq!(
            subscription.result.unwrap()["events"]
                .as_array()
                .unwrap()
                .len(),
            1
