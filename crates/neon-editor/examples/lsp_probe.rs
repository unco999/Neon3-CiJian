...
fn main() {
    ...
    println!("DIAGNOSTICS({}): {}", diagnostics.len(), serde_json::to_string(&diagnostics).unwrap());
    ...
    match lsp.request_definition(uri, Position::new(2, 13)) {
        Ok(locations) => println!("DEFINITION({}): {}", locations.len(), serde_json::to_string(&locations).unwrap()),
        Err(error) => println!("DEFINITION_ERR: {error}"),
    }

    // References of `add`.
    match lsp.request_references(uri, Position::new(0, 4)) {
        Ok(locations) => println!("REFERENCES({}): {}", locations.len(), serde_json::to_string(&locations).unwrap()),
        Err(error) => println!("REFERENCES_ERR: {error}"),
    }

    // Document outline.
    match lsp.request_symbols(uri) {
        Ok(symbols) => println!("SYMBOLS({}): {}", symbols.len(), serde_json::to_string(&symbols).unwrap()),
    ...
    match lsp.request_signature_help(uri, Position::new(2, 17)) {
        Ok(result) => println!("SIGNATURE_HELP: {}", serde_json::to_string(&result).unwrap()),
