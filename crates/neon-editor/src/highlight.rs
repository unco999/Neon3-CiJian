...
impl HighlightCache {
    ...
    /// Builds the cache from a pre-tokenized document (tree-sitter path).
    /// Line count must equal the document's line count.
    pub fn rebuild_tokens(tokens: Vec<LineTokens>) -> Self {
        Self { lines: tokens }
    }
