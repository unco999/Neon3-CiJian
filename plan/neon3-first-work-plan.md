...
2026-08-13 | M9 | 已完成（React forwarding and dirty-frame composition extension）
files: `crates/neon-protocol/src/lib.rs`, `crates/neon-ipc/src/lib.rs`, `crates/neon-ui-runtime/src/lib.rs`, `crates/neon-ui-runtime/src/main.rs`, `crates/neon-wgpu-runtime/src/lib.rs`, `crates/neon-wgpu-runtime/src/main.rs`, `crates/neon-wgpu-runtime/src/ui_renderer.rs`, `packages/neon-ui-react-client/*`, `plan/neon3-ui-react-client.md`, `plan/neon3-first-work-plan.md`
checks: `cargo test -p neon-ipc -p neon-protocol -p neon-ui-runtime` = passed (IPC `9`, protocol `8` plus `6` integration contracts, UI runtime `9`); targeted WGPU `react_client_cannot_bypass_ui_runtime_to_submit_a_fragment`, `window_mailbox_ignores_stale_composition_and_idles_without_dirty_work`, and `animation_activity_expires_after_transition_end` = passed; `npm test` in `packages/neon-ui-react-client` = passed (`6` tests); actual loopback flow = passed: `neon-wgpu-runtime --window-server 127.0.0.1:40101`, `neon-ui-runtime --forward-server 127.0.0.1:40100 127.0.0.1:40101`, then `npm run demo:terrain -- 40100` returned accepted request `833abeca-79a4-4748-8aa3-c96b9a4c5017`; public snapshot query confirmed WGPU composition revision `1` and terrain fragment revision `42`; no `git diff --check` errors
commit: none (working tree contains independent user/parallel modifications)
remaining: the runtime window receives the complex declaration and draws its geometry. Window text remains dependent on the revisioned project-owner font preload path; no system/declaration fallback was introduced. Full UI Runtime service discovery, subscriptions, semantic event return path, and projectd asset service remain later service lifecycle work.
next: add projectd-backed font/resource preload forwarding to the window renderer before treating text-rich interactive composition as `wgpu-rendered`; retain event-driven redraw and monotonic mailbox semantics.
user_acceptance: interactive window explicitly authorized and left running for user inspection; automated result is composition-ready, not a declaration of interactive acceptance

2026-08-13 | M9 | 已完成（P0 typography fixture revalidation）
files: `assets/fonts/SarasaUiSC-Light.ttf`, `assets/fonts/SarasaUiSC-Light.NOTICE`, `assets/fonts/LICENSE-Sarasa-Gothic-OFL-1.1.txt`, `crates/neon-projectd/src/lib.rs`, `crates/neon-wgpu-runtime/src/ui_renderer.rs`, `plan/neon3-font-residency.md`, `plan/neon3-first-work-plan.md`
checks: `Get-FileHash -Algorithm SHA256 assets/fonts/SarasaUiSC-Light.ttf` = passed (`57AE267C048B47CDA7CCCA9E8834686EC046217EC43715039015F1852FE394CC`, `24,156,672` bytes); `cargo test -p neon-wgpu-runtime ui_renderer::tests::owner_font_and_text_ref_produce_glyph_pixels_without_background_fill -- --exact --nocapture --test-threads=1` = passed; `cargo test -p neon-wgpu-runtime tests::project_font_preload_job_drives_private_text_glyph_residency -- --exact --nocapture --test-threads=1` = passed; `cargo test -p neon-wgpu-runtime ui_renderer::tests::text_wraps_within_label_width_and_respects_parent_clip -- --exact --nocapture --test-threads=1` = passed; `RUST_TEST_THREADS=1 cargo test -p neon-ui-schema -p neon-ui-runtime -p neon-wgpu-runtime -p neon-cli` = passed (schema `16`, UI runtime `6`, WGPU runtime `34`, CLI `3`, doctests); `git diff --check` = passed; no window launched
commit: none (working tree contains independent user/parallel modifications)
remaining: the P0 coverage atlas is valid for bootstrap text and tests, not the final full-CJK product text pipeline. The ordinary parallel WGPU test harness still intermittently exits with Windows `STATUS_ACCESS_VIOLATION`; serial execution is the current deterministic gate for headless WGPU tests. `plan/neon3-font-residency.md` records the separate GPK/SDF design required before full product font residency.
next: before a product-scale text milestone, define versioned `font/gpk` asset/schema, offline compiler, license/source-hash metadata, bounded atlas residency policy, and GPU acceptance scenario; do not migrate Neon2 code or use font paths/mmap/GPU handles in the public protocol.
user_acceptance: not requested; no window launched
